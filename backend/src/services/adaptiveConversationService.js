...
export function extractAdaptiveSignals(customerMessage, deal, conversationHistory = []) {
  ...
  const pricingRequested = /\b(how much|what(?:'s| is) the price|pricing|price|quote|cost|expensive|cheaper|budget)\b/i.test(lower);
  ...
  const enterpriseRequirement = (users !== null && users >= 200) || /\b(enterprise|global rollout|multi[- ]region|complex integration|custom contract)\b/i.test(lower);
  const specialPricing = requestedDiscountPct !== null || /\b(special pricing|custom pricing|approval|approve|exception|concession)\b/i.test(lower);
  ...
  if (asksForHuman) {
  ...
  } else if (asksToRecall && hasPriorContext) {
    ...
    intent = /\b(how much|what is the price|pricing|quote|cost)\b/i.test(lower)
  ...
  if (pricePressure) objections.push({ type: OBJECTION_TYPES.PRICE, text: 'Customer raised a price, budget, or value concern.' });
...
export function buildAdaptiveResponse({ customerMessage, deal, analysis, conversationHistory = [] }) {
  ...
  if (analysis.requirementChanged) {
    return `Understood. I'll update your requirement to ${users} users. At that scale, the ${quote.planName.toLowerCase()} may be more suitable. For ${users} users, the estimated annual price is ${formatMoney(quote.finalAmount, quote.currency)}. Would you like me to prepare the enterprise proposal or schedule a technical walkthrough?`;
  }
  ...
  if (analysis.intent === SALES_INTENTS.OBJECTION && analysis.objections?.some((objection) => objection.type === OBJECTION_TYPES.PRICE)) {
    if (analysis.pricePriorityAlreadyKnown || analysis.priority === 'pricing') {
      const priorityQuote = getConfiguredQuote(deal, pricingUsers, 0, analysis.commitmentMonths);
      return `Understood. I'll prioritize pricing in the comparison. For your current requirement of ${pricingUsers} users, our estimated annual price is ${formatMoney(priorityQuote.finalAmount, priorityQuote.currency)}. Salesforce may have lower-priced editions, but the right comparison depends on the features and capabilities you need. Would you like me to prepare a side-by-side comparison?`;
    }
    if (competitor && /\b(salesforce)\b/i.test(competitor)) {
      return `I understand. Since price is important to you, let's look at the numbers and overall value for your ${pricingUsers}-user requirement. Would you like me to compare the pricing and key differences with Salesforce?`;
    }
  ...
  if (analysis.intent === SALES_INTENTS.PRICING_INQUIRY || analysis.pricingRequested) {
    return `For ${pricingUsers} users, the estimated annual price for the ${quote.planName} is ${formatMoney(quote.finalAmount, quote.currency)}. This includes ${quote.negotiationBounds.planFeatures.join(', ')}. Based on your requirements, this looks like a suitable option. Would you like me to prepare a tailored proposal or schedule a product walkthrough?`;
  }
